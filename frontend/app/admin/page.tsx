...
export default function AdminPage() {
  ...
  const openExamDetail = useCallback(async (examId: number) => {
    setExamDetailOpen(true);
    setExamDetailLoading(true);
    setExamDetailError(null);
    setExamDetail(null);
    try {
      const d = await getExamDetailAdmin(examId);
      setExamDetail(d);
    } catch (e) {
      setExamDetailError(e instanceof Error ? e.message : "Could not load exam detail");
    } finally {
                          ...
                          return (
                            <tr key={exam.id} className="bg-white dark:bg-zinc-900">
                              <td className="max-w-[200px] px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">
                                <button
                                  type="button"
                                  onClick={() => void openExamDetail(exam.id)}
      ...
      {examDetailOpen && (
        ...
        >
          <div
            ...
            className="absolute right-0 top-0 h-full w-full max-w-2xl overflow-hidden border-l border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-900"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex h-full flex-col">
              <div className="flex shrink-0 items-start justify-between gap-4 border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                <div className="min-w-0">
                  <h2 id="exam-detail-title" className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">
                    {examDetailLoading ? "Loading exam..." : examDetail?.exam_title ?? examDetail?.exam_topics.join(", ") ?? "Exam detail"}
                  </h2>
                  {examDetail && (
                    <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                      {examDetail.total_questions} questions · {examDetail.exam_complexity} · Created{" "}
                      {new Date(examDetail.created_at).toLocaleString()}
                    </p>
                  )}
                </div>
                <button
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
                ...
                {examDetail && !examDetailLoading && (
                  <ol className="space-y-4">
                    {examDetail.questions.map((q, qi) => (
                      <li key={q.question_id} className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-zinc-100 pb-3 dark:border-zinc-800">
                          <span className="text-xs font-bold uppercase tracking-wide text-zinc-500">
                            Question {qi + 1}
                          </span>
                            ...
                            return (
                              <li
                                ...
                                className={`flex items-start gap-3 rounded-lg border px-3 py-2.5 text-xs ${
                                  isCorrectOpt
                                    ? "border-emerald-500 bg-emerald-50/90 dark:border-emerald-600 dark:bg-emerald-950/35"
                                    : "border-zinc-200 bg-zinc-50/50 dark:border-zinc-700 dark:bg-zinc-800/30"
                                }`}
                              >
                                <span
